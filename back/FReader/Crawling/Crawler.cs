...
namespace Freader.Crawling
{
    public class Crawler
    {
        ...
        {
            //尝试从本地获取数据
            StorageBook book = Localizer.GetBook(bid);
            //检验本地数据是否合格
            if (book != null
                //保证资料完整度足够
                && book.InfoLevel.Value >= InfoLevel.Detail.Value
                //保证资料的来源是要求的源
                && book.Source == source
                //保证资料足够新
            ...
            if (book != null)
                ...
                resources = Localizer.GetBookResourceInfo(bid, source);
            if (resources.Length == 0)
                return new DetailResult() { Error = "获取书籍数据失败，请重新搜索书籍" };
            return GetResourceProvider(source).Detail(bid, resources[0], syncPersist);
        }
        /// <summary>
        /// 获取书籍目录
        /// </summary>
        ...
        {
            //尝试从本地获取数据
            StorageCatalog storageCatalog = Localizer.GetCatalog(bid, source);
        ...
        {
            //尝试从本地获取数据
            StorageChpater storageChpater = Localizer.GetChapter(cid);
            if (storageChpater != null)
                return new ChapterResult() { Chapter = storageChpater };
            //尝试从远程数据源获取数据
            ResourceInformation chapterInfo = Localizer.GetChapterResourceInfo(bid, cid, source);
            if (chapterInfo != null)
            ...
            //再次尝试从远程数据源获取数据
            chapterInfo = Localizer.GetChapterResourceInfo(bid, cid, source);
            if (chapterInfo != null)
                return GetResourceProvider(source).Chapter(bid, chapterInfo, syncPersist);
